...
class BigramTester(object):
    ...
    def compute_entropy_cumulatively(self, word):
        ...
        if word not in self.unigram_count:
            ...
            if self.test_words_processed == 0: #First iteration when having no word prev
                p0 = self.unigram_count[word]/self.total_words
                self.logProb -= math.log(p0*self.lambda2 + self.lambda3) #Adding the first word log prob IP
                
            else:
                if self.last_index == -1 or word not in self.bigram_prob[self.index[self.last_index]]:
                    p0 = self.unigram_count[word]/self.total_words
                    p1 = p0*self.lambda2 + self.lambda3
                    self.logProb -= math.log(p1)
                else:
                    p0 = self.unigram_count[word]/self.total_words
                    p01 = math.exp(self.bigram_prob[self.index[self.last_index]][word])
                    p1 = p01*self.lambda1 + p0*self.lambda2 + self.lambda3
                    self.logProb -= math.log(p1) 
            self.last_index = self.word[word]
...
def main():
    ...
    if arguments.check:
        ...
        payload = json.dumps({
            'model': open(arguments.file, 'r', encoding="utf8").read(),
