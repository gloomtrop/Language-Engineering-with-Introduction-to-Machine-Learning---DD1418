...
class RandomIndexing(object):
    ...
    ##
    def __init__(self, filenames, dimension=2000, non_zero=100, non_zero_values=list([-1, 1]), left_window_size=3, right_window_size=0):
    ...
    def clean_line(self, line):
        # YOUR CODE HERE
        return line.translate(str.maketrans("", "", string.punctuation + string.digits)).split()
    ...
    def build_vocabulary(self):
        ...
        for clean_line in self.text_gen():
            self.__vocab.update(clean_line)
    ...
    def create_word_vectors(self):
        ...
        for word in self.__vocab:
            ...
            self.__rv[word] = np.zeros(self.__dim-self.__non_zero, dtype=int)
                
            non_zeros = random.choices(self.__non_zero_values, k = self.__non_zero)
            self.__rv[word] = np.concatenate((non_zeros, self.__rv[word]))
            np.random.shuffle(self.__rv[word])

        for cline in self.text_gen():
            t = 0
            for word in cline:
                lws, rws = self.__lws, self.__rws
                if t-self.__lws < 0:
                    lws = t
                if t+self.__rws > len(cline)-1:
                    rws = len(cline)-1-t
                for j in range(-lws, rws+1): #(-3) - 0 -3
                    if j != 0:
                        self.__cv[word] += self.__rv[cline[t+j]]
                t += 1
    ...
    def find_nearest(self, words, k=5, metric='cosine'):
        # YOUR CODE HERE
        X, word_l, key_index, non_existing = list(), list(), list(), list()
        for key, val in self.__cv.items():
            X.append(val)
            key_index.append(key)
        for i in range(len(words)):
            try:
                word_l.append(self.__cv[words[i]])
            except KeyError:
        ...
        if word_l:
            ...
            nn.fit(X)
            nnds,nnis  = nn.kneighbors(word_l)

            for i in range(len(nnis)):
                nnb = list()
                for nni in nnis[i]:
                    nnb.append(key_index[nni])
                nnbs.append(list(zip(nnb, np.round(nnds[i], 2))))

        for i in non_existing:
            nnbs.insert(i, "The word does not exist")
        return nnbs
